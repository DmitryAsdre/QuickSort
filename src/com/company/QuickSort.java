...
public class QuickSort {
    public static void quick_sort(int[] arr, int from_Index, int to_Index){
        if(from_Index < to_Index){
            if (from_Index - to_Index  < 4){
                instertion_sort(arr, from_Index, to_Index);
            }else {
    ...
    private static void instertion_sort(int[] arr, int from_Index, int to_Index){
        for(int i = from_Index + 1; i < to_Index; i++){
            int j = i - 1;
            while(j >= from_Index && arr[j] > arr[j + 1]){
